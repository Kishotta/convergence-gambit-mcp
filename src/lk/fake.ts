...
export class FakeLegendKeeperClient implements LegendKeeperClient {
  ...
  private seedResource(input: CreateResourceInput, overviewText?: string): Resource {
    ...
    const overview: LKDocument = {
      ...
      title: "Overview",
      infobox: true,
      content: overviewText ?? "Stub — awaiting migration from quarry file.",
    };
  ...
  async listResources(): Promise<ResourceSummary[]> {
    return [...this.rows.values()].map(({ resource: { documents, ...summary } }) => summary);
  }
  ...
  async updateResource(resourceId: string, patch: UpdateResourcePatch): Promise<Resource> {
  ...
  async batchCreateResources(inputs: CreateResourceInput[]): Promise<Resource[]> {
    return Promise.all(inputs.map((input) => this.createResource(input)));
  }

  async getDocument(resourceId: string, documentId: string): Promise<LKDocument> {
    const doc = this.row(resourceId).documents.get(documentId);
    if (!doc) throw new Error(`Document not found: ${documentId}`);
    return doc;
  }

  async createDocument(resourceId: string, input: CreateDocumentInput): Promise<LKDocument> {
    const row = this.row(resourceId);
