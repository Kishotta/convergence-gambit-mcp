/**
 * CSRF token, signed-cookie, and approval-dialog helpers for the OAuth
 * authorize/callback flow. Ported near-verbatim from Cloudflare's
 * remote-mcp-github-oauth reference (fetched 2026-07-08, not from training
 * data). Security-critical and already hardened (CSRF binding, HMAC-signed
 * cookies, RFC 9700 one-time-use state) — don't reinvent this.
 */
 ...
 *
 * Uses whitelist security: only http/https schemes are allowed. All other
 * schemes (javascript:, data:, file:, etc.) are rejected.
 *
 * NOTE: this only validates URL structure and scheme. It does NOT perform
 * HTML escaping — call sanitizeText() on the result before using it in HTML.
 */
...
export function validateCSRFToken(formData: FormData, request: Request): ValidateCSRFResult {
    ...
    // RFC 9700: CSRF tokens must be one-time use — clear the cookie to prevent reuse.
 ...
 * is the same browser that consented to the authorization request. The state
 * token is hashed rather than stored directly for defense-in-depth: even if
 * the state parameter leaks (URL logs, referrer headers), the cookie value
...
export async function validateOAuthState(
    ...
    // Validate that this state token belongs to this browser session by
    // checking that the state hash matches the session cookie.
    const cookieHeader = request.headers.get("Cookie") || "";
    const cookies = cookieHeader.split(";").map((c) => c.trim());
...
export interface ApprovalDialogOptions {
    ...
    /**
     * Arbitrary state data to pass through the approval flow. Will be
     * encoded in the form and returned when approval is complete.
     */
 ...
 * The dialog displays information about the client and server and includes
 * a form to submit approval with CSRF protection.
 */
